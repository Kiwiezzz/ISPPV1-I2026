...
class Pong(Game):
    ...
    def on_input(self, input_id: str, input_data: InputData) -> None:
        if input_id == "quit" and input_data.pressed:
            ...
            self.player1.y = settings.PADDLE_Y_OFFSET
            self.player2.y = settings.VIRTUAL_HEIGHT - settings.PADDLE_HEIGHT - settings.PADDLE_Y_OFFSET
            self.ball.x = settings.VIRTUAL_WIDTH/2 - settings.BALL_SIZE/2
            self.ball.y = settings.VIRTUAL_HEIGHT/2 - settings.BALL_SIZE/2
            self.player1_score = 0
            self.player2_score = 0
            self.serving_player = 1
            self.winning_player = 0
