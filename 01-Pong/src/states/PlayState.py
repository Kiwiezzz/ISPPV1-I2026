...
class PlayState(BaseState):
    ...
    def update(self, dt: float) -> None:
        ...
        # Calculate the direction the CPU should steer the paddle after the ball 
        # covers a certain distance, just as a human would.
        if(pong.cpu1_active and pong.ball.vx > 0 and pong.ball.x > settings.VIRTUAL_WIDTH/4):
            distance_y = ((pong.ball.y + pong.ball.height/2) - (pong.player2.y + pong.player2.height/2))
            # The paddle swings abruptly (unnatural movement) if it is not
            # given a tolerance to which it must act
            if distance_y < -settings.CPU_TOLERANCE:
                pong.player2.vy = -settings.PADDLE_SPEED
            elif distance_y > settings.CPU_TOLERANCE:
                pong.player2.vy = settings.PADDLE_SPEED
            else:
                pong.player2.vy = 0

        if(pong.cpu2_active and pong.ball.vx < 0 and pong.ball.x < (settings.VIRTUAL_WIDTH/4)* 3):
            distance_y = ((pong.ball.y + pong.ball.height/2) - (pong.player1.y + pong.player1.height/2))
            if distance_y < -settings.CPU_TOLERANCE:
